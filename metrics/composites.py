...
class MusicAVQACompositeMetric(BaseMetric):
    ...
    def compute(self, forward_pass=None, dataloader=None, num_classes=42, **_):
        ...
        if preds.numel() != labels.numel():
            ...
            types = self._types
        out = {'accuracy': Fm.accuracy(preds, labels), 'macro_f1': Fm.macro_f1_multiclass(preds, labels, num_classes), 'by_type': Fm.by_question_type(preds, labels, types)}
...
class ClassificationCompositeMetric(BaseMetric):
    ...
    def compute(self, num_classes=42, **_):
        ...
        return {'accuracy': Fm.accuracy(preds, labels), 'macro_f1': Fm.macro_f1_multiclass(preds, labels, num_classes), 'top3_acc': (topk == labels.unsqueeze(1)).any(dim=1).float().mean().item()}
