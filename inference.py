...
def _append_results_row(config, logs, save_path):
    ...
    row = {'timestamp_utc': datetime.utcnow().isoformat(timespec='seconds'), 'dataset': dataset_name, 'model': model_target, 'checkpoint': checkpoint_path, 'save_path': str(save_path)}
    ...
    dynamic_cols = sorted({key for r in all_rows for key in r.keys() if key not in base_cols})
