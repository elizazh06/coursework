...
class ADVANCEBaselineModel(nn.Module):
    ...
    def __init__(self, num_classes=13, hidden_dim=256, dropout=0.2, use_pretrained_backbone=True):
        ...
        self.audio_encoder = nn.Sequential(nn.Conv1d(128, hidden_dim, kernel_size=5, padding=2), nn.ReLU(), nn.Conv1d(hidden_dim, hidden_dim, kernel_size=3, padding=1), nn.ReLU())
        self.fusion = nn.Sequential(nn.Linear(hidden_dim * 2, hidden_dim), nn.ReLU(), nn.Dropout(dropout), nn.Linear(hidden_dim, num_classes))
