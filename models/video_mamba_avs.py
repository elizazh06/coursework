...
class VideoMambaAVSModel(BaseSSMAVSModel):
    ...
    def __init__(self, d_model: int=768, n_visual_layers: int=2, n_fusion_layers: int=2, d_state: int=16, conv_kernel: int=4, expand: int=2, ff_mult: int=4, dropout: float=0.1, max_audio_tokens: int=10, max_text_tokens: int=25, mask_size: int=256, audio_dim: int=128, text_dim: int=768, pretrained_visual_model: str | None='facebook/mask2former-swin-base-ade-semantic', freeze_visual_backbone: bool=True, use_official_mamba_ssm: bool=True, hf_pretrained_mamba_model: str | None='state-spaces/mamba-130m-hf', pretrained_mamba_path: str | None=None, pretrained_mamba_prefix: str='', freeze_mamba: bool=False, **_):
        ...
        if hf_pretrained_mamba_model:
            self._load_pretrained_mamba_from_hf(hf_pretrained_mamba_model, ('visual_blocks', 'fusion_blocks'))
        if pretrained_mamba_path:
            self._load_pretrained_mamba_checkpoint(pretrained_mamba_path, pretrained_mamba_prefix, ('visual_blocks.', 'visual_blocks_norm.', 'fusion_blocks.', 'fusion_blocks_norm.'))
        if freeze_mamba:
            for module in (self.visual_blocks, self.visual_blocks_norm, self.fusion_blocks, self.fusion_blocks_norm):
