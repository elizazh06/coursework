...
class MambaVLModel(BaseAVSceneModel):
    ...
    def __init__(self, d_model=768, n_vl_layers=3, n_fusion_layers=1, d_state=16, conv_kernel=4, expand=2, ff_mult=4, dropout=0.1, max_audio_tokens=10, max_text_tokens=25, mask_size=256, image_size=256, audio_dim=128, text_dim=768, pretrained_visual_model='facebook/mask2former-swin-base-ade-semantic', freeze_visual_backbone=True, use_official_mamba_ssm=True, hf_pretrained_mamba_model='state-spaces/mamba-130m-hf', pretrained_mamba_path=None, pretrained_mamba_prefix='', freeze_mamba=False, **_):
        ...
        self.vl_pos_embed = nn.Parameter(torch.zeros(1, max_text_tokens + max_video_tokens, d_model))
        self.fusion_pos_embed = nn.Parameter(torch.zeros(1, max_audio_tokens + max_video_tokens + 10, d_model))
    ...
    def forward(self, frames, audio_feat, text_feat=None, masks=None, **_):
        ...
        video_text_tokens = vl[:, text_tokens.size(1):text_tokens.size(1) + video_tokens.size(1)]
        queries = self.query_embed.expand(b, t, -1)
        fusion = torch.cat([audio_tokens + self.modality_embed[:, 1:2], video_text_tokens + self.modality_embed[:, 0:1], queries + self.modality_embed[:, 3:4]], dim=1)
        fusion = self.dropout(fusion + self.fusion_pos_embed[:, :fusion.size(1)])
        for block in self.fusion_blocks:
            fusion = block(fusion)
        fusion = self.fusion_blocks_norm(fusion)
        v_start = audio_tokens.size(1)
        spatial = fusion[:, v_start:v_start + video_text_tokens.size(1)].view(b, t, h, w, d).permute(0, 1, 4, 2, 3).contiguous()
        q_start = v_start + video_text_tokens.size(1)
