"""Mixture-of-Mamba model for Ref-AVS-Bench referring segmentation.

Reuses Mamba + MoE blocks from ``mixture_of_mamba`` while keeping the Ref-AVS
dataset contract (``frames``, ``audio_feat``, ``text_feat`` → ``logits``).
"""
...
class _MaskDecoder(nn.Module):
    """Fuse a sequence token with a spatial feature map and upsample to mask_size."""
    ...
    def forward(self, token: torch.Tensor, spatial: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            token:   [B, d_model]
            spatial: [B, d_model, h, w]
        Returns:
            logits: [B, mask_size, mask_size]
        """
...
class MixtureOfMambaRefAVSModel(nn.Module):
    """
    Audio-visual referring segmentation with a Mamba-style multimodal backbone.

    Pipeline per clip:
      1. Encode each frame → spatial map + pooled video token.
      2. Project pre-extracted audio / text features to d_model.
      3. Run causal Mamba blocks over
         [video_t, audio_t, text, mask_query_t] tokens.
      4. Decode per-frame masks from mask-query states + spatial maps.

    Returns:
        {"logits": Tensor[B*T, mask_size, mask_size]}
    """
    ...
    def __init__(
        ...
        audio_dim: int = 128,
        text_dim: int = 768,
        max_text_tokens: int = 25,
        mask_size: int = 256,
        image_size: int = 256,
        use_official_mamba_ssm: bool = True,
        hf_pretrained_mamba_model: str | None = "state-spaces/mamba-130m-hf",
        pretrained_mamba_path: str | None = None,
        pretrained_mamba_prefix: str = "",
        freeze_mamba: bool = False,
        **_,
    ):
        super().__init__()
        ...
        if use_official_mamba_ssm:
            try:
                ...
                raise ImportError(
                    "Official Mamba backend is enabled but `mamba-ssm` is missing. "
    ...
    def _encode_frames(self, frames: torch.Tensor):
        """frames [B, T, C, H, W] → spatial [B*T, D, h, w], tokens [B, T, D]."""
    ...
    def forward(
        ...
        **_,
        ...
        del masks  # used by loss / metrics, not inside the model
        ...
        # mask-query states sit after video + audio + text tokens
        t_a = audio_tokens.size(1)
        t_t = min(text_tokens.size(1), self.max_text_tokens)
        mq_start = t + t_a + t_t
        mq_states = x[:, mq_start : mq_start + t]  # [B, T, D]

        spatial_maps = spatial.view(b, t, *spatial.shape[1:])
        logits_list = []
        for f in range(t):
            logits_list.append(
                self.mask_decoder(mq_states[:, f], spatial_maps[:, f])
            )
        logits = torch.cat(logits_list, dim=0)  # [B*T, H, W]
        return {"logits": logits}

    # ------------------------------------------------------------------
    # Pretrained Mamba weight loading (same key mapping as MixtureOfMambaModel)
    # ------------------------------------------------------------------
    ...
    def _load_pretrained_mamba(self, pretrained_path: str, prefix: str = ""):
        ...
        if prefix:
            ...
            src_state = {
                k[len(pref) :]: v for k, v in src_state.items() if k.startswith(pref)
            }
        ...
        to_load = {
            ...
            if k.startswith(("blocks.", "norm."))
            and k in dst_state
            and dst_state[k].shape == v.shape
        }
        if to_load:
            self.load_state_dict(to_load, strict=False)
            print(
                f"[MixtureOfMambaRefAVSModel] Loaded {len(to_load)} tensors "
    ...
    def _load_pretrained_mamba_from_hf(self, hf_model_name: str):
        ...
        try:
            ...
            print(
                f"[MixtureOfMambaRefAVSModel] HF load failed ({e}); "
                "continuing with random Mamba init."
            )
            return
        ...
        if mapped:
            ...
            print(
                f"[MixtureOfMambaRefAVSModel] Loaded {len(mapped)} tensors "
                f"from HF: {hf_model_name}"
            )
