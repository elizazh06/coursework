...
def _pack(item, idx, root, mx, st, strict, uf, dcfg, fad, fvd, w2i, a2i):
    ...
    meta = {'question_id': item.get('question_id', idx), 'question_type': item.get('type', '[]'), 'video_id': vid}
    return (stt, li, meta)
...
class MusicAVQADataset(BaseDataset):
    ...
    def __init__(self, root_dir, split='train', max_len=16, prepare_data=True, download=None, strict_media=False, max_samples=None, answer_to_idx=None, word_to_idx=None, use_official_features=True, features=None, frame_stride=6, limit=None, shuffle_index=False, instance_transforms=None, **kwargs):
        ...
        if not ix_path.exists():
            ...
            index = _materialize(self.root_dir, split, rows, word_to_idx, answer_to_idx, max_len, frame_stride, strict_media, use_official_features, dl, feats)
            if split == 'train':
    ...
    def __getitem__(self, ind):
        ...
        out = {'video': t['video'], 'audio': t['audio'], 'question': t['question'], 'label': t['label'].reshape(-1)[0].long(), 'question_id': e['question_id'], 'question_type': e['question_type'], 'video_id': e['video_id']}
    ...
    def collate_batch(batch):
        ...
        return {'video': video, 'audio': audio, 'question': torch.stack(pad), 'label': labels, 'question_id': [s.get('question_id', -1) for s in batch], 'question_type': [s.get('question_type', '[]') for s in batch], 'video_id': [s.get('video_id', '') for s in batch]}
