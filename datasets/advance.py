...
class ADVANCEDataset(BaseDataset):
    def __init__(
        ...
        **_,
        ...
        base_cache_root = (
            ...
            else self.vision_root.parent / ".advance_cache"
        )
    ...
    def _build_or_load_cache_index(self, split_pairs):
        ...
        if should_rebuild and not self.prepare_cache_if_missing:
            raise RuntimeError(
                f"ADVANCE cache is missing/invalid at {self.cache_dir}. "
                "Enable `prepare_cache_if_missing` or run one training pass to build it."
            )
        ...
        for i, (v, a, c) in enumerate(tqdm(split_pairs, desc=f"Prepare ADVANCE cache [{self.split}]")):
            ...
            payload = {
                "video": video,
                "audio": audio,
                "label": torch.tensor(self.class_to_idx[c], dtype=torch.long),
                "class_name": c,
            }
