...
def get_dataloaders(config, device):
    ...
    shared_state = {'word_to_idx': getattr(train_dataset, 'word_to_idx', None), 'answer_to_idx': getattr(train_dataset, 'answer_to_idx', None)}
    ...
    dataloaders = {'train': DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=pin_memory, collate_fn=train_dataset.collate_batch, timeout=worker_timeout, persistent_workers=persistent_workers, prefetch_factor=prefetch_factor if num_workers > 0 else None), 'val': DataLoader(val_dataset, batch_size=eval_batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin_memory, collate_fn=val_dataset.collate_batch, timeout=worker_timeout, persistent_workers=persistent_workers, prefetch_factor=prefetch_factor if num_workers > 0 else None), 'test': DataLoader(test_dataset, batch_size=eval_batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin_memory, collate_fn=test_dataset.collate_batch, timeout=worker_timeout, persistent_workers=persistent_workers, prefetch_factor=prefetch_factor if num_workers > 0 else None)}
