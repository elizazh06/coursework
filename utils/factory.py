...
def _filter_params(target, params: dict) -> dict:
    """Return only the kwargs that `target.__init__` actually accepts.

    If the target accepts **kwargs, all params are passed through unchanged.
    This prevents leftover keys from config deep-merges (e.g. when switching
    dataset/model profiles) from causing TypeError on unrecognised arguments.
    """
    try:
