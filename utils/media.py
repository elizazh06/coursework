...
def mel_from_wav_path(path, L, n_mels=128, n_fft=1024, hop=256):
    ...
    m = torchaudio.transforms.MelSpectrogram(sample_rate=sr, n_mels=n_mels, n_fft=n_fft, hop_length=hop)(w).squeeze(0).T
    n = m.size(0)
    if n > L:
        return m[:L]
    if n < L:
...
def extract_wav_from_video(video_path, wav_path):
    ...
    try:
        subprocess.run(['ffmpeg', '-nostdin', '-hide_banner', '-loglevel', 'error', '-y', '-i', str(video_path), '-vn', '-acodec', 'pcm_s16le', '-ar', '16000', '-ac', '1', str(tmp)], check=True, capture_output=True, text=True)
